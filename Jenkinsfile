node {
    stage('Building Docker image'){
        ...
        try {
            sh 'sudo docker exec testcontainer_remote_test pycodestyle--ignore=E501 /opt/python_good.py';
            echo 'python_good.py is in good format';
            }catch(x){
                println "python_good.py is in bad format";
                }
        try {
            sh 'sudo docker exec testcontainer_remote_ex2 pycodestyle--ignore=E501 /opt/python_bad.py';
